...
int main(int argc, char **argv)                     //main takes ipadress and port number as inputs
{
  ...
  Window = SDL_CreateWindow("fonsternamn",
	      ...
	      while(1)
		{
		  if(SDL_TryLockMutex(inc->mutex) ==0)
		      ...
		      if(inclength > 52)
			{
			  for(;inclength > 0;inclength-=52)
			      ...
			      for(i = 15;i > 0;i--)
				{
			      ...
			      memcpy(&inc->Rendertext[0], &buffer2[LENGTH*Rowsplitts], LENGTH);
			      Rowsplitts++;
			    }
			}
		      for(i = 15;i > 0;i--)
			{
		      ...
		      memset(inc->Rendertext[0], 0, 64);
		      memcpy(&inc->Rendertext[0], &buffer2[52*Rowsplitts], strlen(buffer2+(52*Rowsplitts)));
		      Rowsplitts = 0;
